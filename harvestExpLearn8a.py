...
DEBUG_PRINT = False
##=============================================================================================
def dp(line_number, nameStr, object):
	if DEBUG_PRINT:
		print(line_number, nameStr+': ', object)
		
#------------------------
def print_timing(function_name, start_time):
	timeEnd = time.time()
	print("\n-----------------------", function_name, " took	",	round((timeEnd-start_time), 1), " seconds\n" )
	

#------------------------
def run_harvest_orig():
	dp(46, 'Harvester v 4.0 w/mods', None)
	...
	dp(64, 'after extract_features() trainingSets', trainingSets)
	...
	timeEnd = time.time()
	print(80, 'Total harvest processing time:', round((timeEnd-timeStart_1), 1)	)
	print(81, 'Reached the end of harvest...')
	
##---------------------------------------------------------------------
def parse_input():
	dp(50, 'start parse_input()', None)
...
def gather_stopwords(args):
	dp(138, ' start gather_stopwords()', None)
	#open stopwords file
	dict = vars(args)
	stop = dict["stopwords"]
	dp(142, "Stopwords file: ", stop)
...
def extract_features(args, wnl, stopwords, headers):
	...
	if targetFile:
		if (".csv") not in targetFile:
			dp(188, "Target file, %s, must be a csv file", (args.targetFile[0]))
			sys.exit(0)
...
#save raw classification data
	...
	for trainingSet in trainingSets:
		dp(581, 'trainingSets', trainingSets)
		dp(582, 'trainingSet', trainingSet)
		if (".csv") not in trainingSet:
			try:
				...
				dp(606,  'trainingSet: ', trainingSet)
		...
		for traininglabel in trainingLabels:
			dp(611, 'trainingLabel: ', traininglabel)
...
#				  print(memorySpec)
				...
				def setup_otherLabels():
					index = 0
					dp(1470, "otherLabels", otherLabels)
					for otherLabel in otherLabels:
						...
						dp(1473, 'otherDocSamples', otherDocSamplesList[index])
						index = index + 1
				...
				#	print('1478 Start compute_new_memorySpec\n')
					items_count = 0
					for term, (count,tf) in sorted(memorySpec.items()):
						otherSample = 0.0
						df = 0.0
						index2 = 0
						for otherLabel in otherLabels:
							...
							if (items_count <=1):
								dp(1499, 'term', term)
								dp(1500, 'otherSample', otherSample)
								dp(1501, 'df', df)
							items_count += 1
						tfIdf = tf * (math.log(((1.5+otherSample)/(1+df))))
						memorySpec[term] = (tf,tfIdf)
						index2 +=  1
# 					print('1507 items_count-otherSample: ', items_count, "-", otherSample)
# 					print('1508 items_count-term, memorySpec[term]: ',items_count, "-", term, "\t", memorySpec[term])
				...
				if (ii == 0):
					dp(1540, 'memorySpec after: ', memorySpec)
					ii = ii +1
